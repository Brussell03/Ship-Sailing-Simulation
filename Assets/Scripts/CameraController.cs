...
namespace UnityEngine.Rendering {
	...
	public class CameraController : MonoBehaviour {
		...
		/// <summary>
		/// Camera movement and positioning mode. (0 = Free, 1 = Following, 2 = Focused)
		/// </summary>
		...
		private BuoyantHull m_FocusedShip;
		...
		void Update()
		{
				...
				if (m_FocusedObject != null) { // Enter free mode
					...
					if (Physics.Raycast(transform.position, transform.forward, out hit, 200)) {
						...
						if (m_CameraPivot != null) {
							...
							m_CamRotation = new Vector3(Mathf.Clamp(transform.eulerAngles.y, -5.0f, 80.0f), transform.eulerAngles.x, 0.0f);
			...
			// If Camera Mode is in free mode
			if (m_CameraMode < 1) {
		...
		void Orbit() {
			...
			if (inputRotateAxisX != 0.0f || inputRotateAxisY != 0.0f) {
				m_CamRotation.x += inputRotateAxisX;
				m_CamRotation.y -= inputRotateAxisY;

				if (m_CamRotation.x > 360.0f) {
					m_CamRotation.x -= 360.0f;
				} else if (m_CamRotation.x < 0.0f) {
					m_CamRotation.x += 360.0f;
				}

				m_CamRotation.y = Mathf.Clamp(m_CamRotation.y, -5.0f, 80.0f);
			}
